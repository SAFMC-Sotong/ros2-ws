...
namespace stereolabs
{
  ...
  void ZedCamera::getPosTrackingParams()
  {
    ...
    getParam("pos_tracking.enable_light_computation_mode", mLightComputeEnabled, mLightComputeEnabled);
    RCLCPP_INFO_STREAM(
      get_logger(),
      " * Positional tracking GEN2 light computation enabled: "
          << (mLightComputeEnabled ? "TRUE" : "FALSE"));
  ...
  {
    // @note: UOSM commented out the following lines
  ...
  bool ZedCamera::startPosTracking()
  {
    ...
    if(mPosTrkMode == sl::POSITIONAL_TRACKING_MODE::GEN_2)
    {
      ptParams.enable_light_computation_mode = mLightComputeEnabled;
    }
      ...
      try
      {
        ...
        if (!mDepthDisabled)
        {
          ...
          if (mPosTrackingStarted)
          {
            ...
            // @note UOSM: Commented out the following lines as they are not needed
